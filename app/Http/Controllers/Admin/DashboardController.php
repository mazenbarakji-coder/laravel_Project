...
class DashboardController extends BaseController
{
    ...
    public function dashboard(Request|null $request = null): View
    {
        ...
        // Product section data (similar to POS)
        $categoryId = $request ? ($request['category_id'] ?? null) : null;
        $categories = $this->categoryRepo->getListWhere(orderBy: ['id' => 'desc'], filters: ['position' => 0]);
        $searchValue = $request ? ($request['searchValue'] ?? null) : null;
        ...
        return view(Dashboard::VIEW[VIEW], compact('data', 'inHouseEarning', 'vendorEarning', 'commissionEarn','inHouseOrderEarningArray','vendorOrderEarningArray','label','dateType', 'categories', 'categoryId', 'products', 'searchValue'));
    }
