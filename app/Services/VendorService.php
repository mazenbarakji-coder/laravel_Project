...
class VendorService
{
    ...
    public function getVendorDataForUpdate(object $request, object $vendor):array
    {
        ...
        $image = $request['image'] ? $this->update(dir: 'seller/', oldImage: $vendor['image'], format: $imageFormat, image: $request->file('image')) : $vendor['image'];
        return [
    ...
    public function getAddData(object $request):array
    {
        return [
            ...
            'phone' => $request['phone'],
            'email' => $request['email'],
            'image' => $this->upload(dir: 'seller/', format: $this->getSupportedImageFormat(), image: $request->file('image')),
            'password' => bcrypt($request['password']),
