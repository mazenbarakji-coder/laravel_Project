...
class ShopService
{
    ...
    public function getShopDataForUpdate(object $request , object $shop):array
    {
        ...
        $image = $request['image'] ? $this->update(dir:'shop/', oldImage: $shop['image'], format: $imageFormat,image:  $request->file('image')) : $shop['image'];
        $banner = $request['banner'] ? $this->update(dir: 'shop/banner/',oldImage:  $shop['banner'], format: $imageFormat,image:  $request->file('banner')): $shop['banner'];
        $bottomBanner = $request['bottom_banner'] ? $this->update(dir: 'shop/banner/', oldImage: $shop['bottom_banner'], format: $imageFormat, image: $request->file('bottom_banner')) : $shop['bottom_banner'];
        $offerBanner = $request['offer_banner'] ? $this->update(dir: 'shop/banner/', oldImage: $shop['offer_banner'], format: $imageFormat,image:  $request->file('offer_banner')) : $shop['offer_banner'];
        return [
            'name'=>$request['name'],
    ...
    public function getAddShopDataForRegistration(object $request,int $vendorId):array
    {
        ...
        return [
            ...
            'image' => $this->upload(dir: 'shop/', format: $imageFormat, image: $request->file('logo')),
            'image_storage_type' => $request->has('logo') ? $storage : null,
            'banner' => $this->upload(dir: 'shop/banner/', format: $imageFormat, image: $request->file('banner')),
            'banner_storage_type' =>$request->has('banner') ? $storage : null,
            'bottom_banner' => $this->upload(dir: 'shop/banner/', format: $imageFormat, image: $request->file('bottom_banner')),
