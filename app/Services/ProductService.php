...
class ProductService
{
    ...
    public function getProcessedImages(object $request): array
    {
        ...
        if ($request->has('colors_active') && $request->has('colors') && count($request['colors']) > 0) {
            foreach ($request['colors'] as $color) {
                ...
                if ($request->file($img)) {
                    $image = $this->upload(dir: 'product/', format: $imageFormat, image: $request->file($img));
                    $colorImageSerial[] = [
                        'color' => $color_,
                        'image_name' => $image,
                        'storage' => $storage,
        ...
        if ($request->file('images')) {
            foreach ($request->file('images') as $image) {
                $images = $this->upload(dir: 'product/', format: $imageFormat, image: $image);
                $imageNames[] = [
                    'image_name' => $images,
                    'storage' => $storage,
                ];
    ...
    public function getProcessedUpdateImages(object $request, object $product): array
    {
            ...
            foreach ($inputColors as $color) {
                if (!in_array($color, $dbColorImageFinal)) {
                    ...
                    if ($request->file($image)) {
                        $imageFormat = $this->getSupportedImageFormat();
                        $imageName = $this->upload(dir: 'product/', format: $imageFormat, image: $request->file($image));
                        $productImages[] = [
        ...
        if ($request->file('images')) {
            foreach ($request->file('images') as $image) {
                $imageFormat = $this->getSupportedImageFormat();
                $imageName = $this->upload(dir: 'product/', format: $imageFormat, image: $image);
                $productImages[] = [
                    'image_name' => $imageName,
                    'storage' => $storage,
                ];
    ...
    {
        if ($request->has('colors_active') && $request->has('colors') && count($request['colors']) > 0) {
            $colors = ($request['product_type'] ?? 'physical') == 'physical' ? json_encode($request['colors']) : json_encode([]);
        } else {
            $colors = json_encode([]);
        }
        return $colors;
    ...
    public function getAddProductData(object $request, string $addedBy): array
    {
        ...
        $digitalFile = '';
        if (($request['product_type'] ?? 'physical') == 'digital' && $request['digital_product_type'] == 'ready_product' && $request['digital_file_ready']) {
        ...
        return [
            ...
            'unit' => ($request['product_type'] ?? 'physical') == 'physical' ? $request['unit'] : null,
            'digital_product_type' => ($request['product_type'] ?? 'physical') == 'digital' ? $request['digital_product_type'] : null,
            ...
            'choice_options' => ($request['product_type'] ?? 'physical') == 'physical' ? json_encode($this->getChoiceOptions(request: $request)) : json_encode([]),
            'variation' => ($request['product_type'] ?? 'physical') == 'physical' ? json_encode($variations) : json_encode([]),
            ...
            'shipping_cost' => ($request['product_type'] ?? 'physical') == 'physical' ? currencyConverter(amount: $request['shipping_cost'] ?? 0) : 0,
            'multiply_qty' => (($request['product_type'] ?? 'physical') == 'physical') ? ($request['multiply_qty'] == 'on' ? 1 : 0) : 0, //to be changed in form multiply_qty
            'color_image' => json_encode($processedImages['colored_image_names']),
            'images' => json_encode($processedImages['image_names']),
            'thumbnail' => $request->has('image') ? $this->upload(dir: 'product/thumbnail/', format: $this->getSupportedImageFormat(), image: $request['image']) : $request->existing_thumbnail,
            'thumbnail_storage_type' => $request->has('image') ? $storage : null,
            'meta_title' => $request['meta_title'],
            'meta_description' => $request['meta_description'],
            'meta_image' => $request->has('meta_image') ? $this->upload(dir: 'product/meta/', format: $this->getSupportedImageFormat(), image: $request['meta_image']) : $request->existing_meta_image,
        ];
    }
    ...
    public function getUpdateProductData(object $request, object $product, string $updateBy): array
    {
        ...
        if (($request['product_type'] ?? 'physical') == 'digital') {
        ...
        } elseif (($request['product_type'] ?? 'physical') == 'physical' && $product['digital_file_ready']) {
            $digitalFile = null;
            // $this->delete(filePath: 'product/digital-product/' . $product['digital_file_ready']);
        }
        ...
        $dataArray = [
            ...
            'discount' => $request['discount_type'] == 'flat' ? currencyConverter(amount: $request['discount']) : $request['discount'],
            'discount_type' => $request['discount_type'],
            'attributes' => ($request['product_type'] ?? 'physical') == 'physical' ? json_encode($request['choice_attributes']) : json_encode([]),
            'current_stock' => ($request['product_type'] ?? 'physical') == 'physical' ? abs($stockCount) : 999999999,
            'minimum_order_qty' => $request['minimum_order_qty'],
            'video_provider' => 'youtube',
            'video_url' => $request['video_url'],
            'shipping_cost' => ($request['product_type'] ?? 'physical') == 'physical' ? (getWebConfig(name: 'product_wise_shipping_cost_approval') == 1 && $product->shipping_cost == currencyConverter($request->shipping_cost ?? 0) ? $product->shipping_cost : currencyConverter(amount: $request['shipping_cost'] ?? 0)) : 0,
            'multiply_qty' => (($request['product_type'] ?? 'physical') == 'physical') ? ($request['multiply_qty'] == 'on' ? 1 : 0) : 0,
            'color_image' => json_encode($processedImages['colored_image_names']),
            'images' => json_encode($processedImages['image_names']),
            'digital_file_ready' => $digitalFile,
        ...
        if ($request->file('image')) {
            $dataArray += [
                'thumbnail' => $this->update(dir: 'product/thumbnail/', oldImage: $product['thumbnail'], format: $this->getSupportedImageFormat(), image: $request['image'], fileType: 'image'),
...
//            'purchase_price',
    ...
    public function getProductSEOData(object $request, object|null $product = null, string $action = null): array
    {
        if ($product) {
            if ($request->file('meta_image')) {
                ...
                $metaImage = $this->upload(dir: 'product/meta/', format: $this->getSupportedImageFormat(), image: $request['image']);
            } else {
                $metaImage = $product?->seoInfo?->image ?? $product['meta_image'];            }
        } else {
            if ($request->file('meta_image')) {
                $metaImage = $this->upload(dir: 'product/meta/', format: $this->getSupportedImageFormat(), image: $request['meta_image']);
            } elseif (!$request->file('meta_image') && $request->file('image') && $action == 'add') {
                $metaImage = $this->upload(dir: 'product/meta/', format: $this->getSupportedImageFormat(), image: $request['image']);
            }
        }
    ...
    private function generateUniqueCode(): string
    {
        do {
            ...
            $exists = ProductModel::where('code', $code)->exists();
        } while ($exists);
