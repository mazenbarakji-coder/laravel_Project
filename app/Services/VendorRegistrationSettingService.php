...
class VendorRegistrationSettingService
{
    ...
    protected function getImageDataProcess($request ,$image,$requestImageName):array
    {
        ...
        if ($imageData) {
            $imageName = $request->file($requestImageName) ? $this->update(dir:'vendor-registration-setting/', oldImage: $imageData, format: $imageFormat, image: $request->file($requestImageName)) : $imageData;
            $storage = $request->file($requestImageName) ? $storage : ($image?->storage ?? $storage) ;
        }else {
            $imageName = $request->file($requestImageName) ? $this->upload(dir:'vendor-registration-setting/', format: $imageFormat, image: $request->file($requestImageName)) :null;
        }
        return [
